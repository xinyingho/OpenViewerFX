...
public class QuadTree {
    ...
    public void reset(int i, int j){
        ...
        while(curLev<len){
            ...
            Integer kVal = level.items.get(index);
            if(kVal!=null){
                val = kVal;
                break;
            }
        ...
        curLev--;
        level = nodeMap[curLev];
        level.items.put(level.index, val);
    ...
    public void incrementValue(){
        ...
        level.items.put(level.index,level.items.get(level.index)+1);
    }
    
    public boolean nextNode() {
        int curLev = currentLevel;
        TreeNode level = nodeMap[curLev];
        int val = level.items.get(level.index);
        curLev--;
        if (curLev < 0) {
            this.value = val;
            return false;
